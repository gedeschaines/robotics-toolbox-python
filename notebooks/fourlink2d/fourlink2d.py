...
"""
import _robot                   # use helper function to locate local RTB for Python robot module
...
rbplot(fl2d, Q)            # rbplot() creates a Matplotlib animation object and saves a handle to it in the 
...
# Let's display the last plot as a controlled video.
...
# **In the following animation, only robot \#2 motion is displayed. This is a current limitation of rbplot() and a fix is being devised.**

# In[9]:

# Lets animate figure 2 (Note: only re-animates 2nd robot because it's animator replaced that of 1st robot)
rbplotAnims2D[fignum]
