...
% 12/18 fixed joint and wrist labels to be nonpersistent 
...
function rnew = plot(robot, tg, varargin)
	...
	if ~isfield(robot, 'handles'),
		...
		%%%%%%%%%%%%%% process options
		erasemode = 'xor';  %%% NOTE: erasemode is deprecated in Octave and MATLAB
		joints = 1;
		wrist = 1;
		repeat = 1;
		shadow = 1;
	...
	if isempty(robot.handle) & isempty(findobj(gcf, 'Tag', robot.name)),
		...
		if wrist == 1,	
			...
			h.z = line('xdata', [0;0], ...
				...
				'color', 'blue', ...
				'erasemode', 'xor');
			h.xt = text(0, 0, 0, wristlabel(1), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');
			h.yt = text(0, 0, 0, wristlabel(2), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');
			h.zt = text(0, 0, 0, wristlabel(3), 'FontWeight', 'bold', 'HorizontalAlignment', 'Center');
		end

		%
		% display cylinders (revolute) or boxes (pristmatic) at
		% each joint, as well as axis centerline and label
		%
		if joints == 1,
			L = robot.links;
			for i=1:robot.n,
