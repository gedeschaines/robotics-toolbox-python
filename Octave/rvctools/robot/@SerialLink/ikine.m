...
function [qt, history] = ikine(robot, tr, varargin)
    ...
    if opt.debug ~= 0, disp("ikine: "), varargin, end
    ...
    J0 = jacob0(robot, q);  % jacob0 passes a [1 x n] to fkine; returns a [6 x n]
    J0 = J0(m, :);
    if cond(J0) > 100
    ...
    for i=1:npoints
        ...
        save.eprev = [Inf; Inf; Inf; Inf; Inf; Inf];
        save.q = [];
        count = 0;
        ...
        while true
            ...
            % compute the error
            Tq = fkine(robot, q');  % fkine expects an [n x 1] or an [npoints x n]
            
            e(1:3) = transl(T - Tq);
            Rq = t2r(Tq);
            [th, nv] = tr2angvec(Rq'*t2r(T));
            e(4:6) = th*nv;
            ...
            J = jacob0(robot, q);  % jacob0 passes a [1 x n] to fkine; returns a [6 x n]
    ...
    end  % for i=1:npoints loop
    
    if (opt.verbose > 0) && (npoints > 1)
